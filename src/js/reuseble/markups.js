...
export async function makeMarkup(moviesList, mobileCount = 100) {
  ...
  return moviesList
    .map(({ id, poster_path, title, genre_ids, release_date, vote_average }, i) => {
...
<li class="film-card ${hideMobile}">
  <a href="#" class="film-card-link" movie_id="${id}">
    <img class="film-card-img" src="${wrapPosterPath(poster_path)}" alt="${title}" loading="lazy" />
...
export function createModalMarkup({ title, poster_path, vote_average, vote_count, popularity, genres, overview }) {
  return `<div class="modal">
      ...
      <img src="${wrapPosterPath(poster_path)}" alt="${title}" class="modal__img" />
      <div class="modal__wrap">
        ...
        <ul class="modal-list">
          ...
          <li class="modal-list__item">
            ...
            <span class="modal-list__genres">${genres.map(({ name }) => name).join(', ')}</span>
          </li>
        </ul>
