...
function loadHeroMarkup(data) {
  const heroMarkup = data
    .map(({ id, title, backdrop_path, vote_average, overview }) => {
      return `
           ...
           <button class="watch-trailer" type="button" data-id="${id}">Watch trailer</button>
           <button class="more-details" type="button" data-id="${id}">More details</button>
