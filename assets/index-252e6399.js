import{b as f,g as h,d as y,c as v,f as M,a as x}from"./footer-abc308f2.js";async function L(){const s=[];for(let o=1;o<=5;o+=1){const e=`https://api.themoviedb.org/3/movie/upcoming?api_key=183c3cacc9c38c09c14d38798ccfe9d7&language=en-US&page=${o}`;try{const{data:n}=await f.get(e);s.push(...n.results)}catch(n){console.error(n)}}return s}async function S(t){try{return(await h()).filter(e=>t.includes(e.id)).map((e,n)=>n===0?e.name:e.name.toLowerCase()).join(", ")}catch(s){console.log(s)}}const $="/js-team-project-12/assets/no-image-816a65ed.jpg",b=document.querySelector(".js-upcoming");function w(t){return new Date(t).toLocaleString("uk",{day:"numeric",month:"numeric",year:"numeric"})}function D({backdrop_path:t,overview:s,popularity:o,release_date:e,title:n,vote_average:p,vote_count:g},u){const l=t?`https://image.tmdb.org/t/p/original/${t}`:$,m=s||"Sorry! No description.";return`
        ...
        <div class="upcoming-thumb">
          <ul class="upcoming-list left">
            <li class="upcoming-list__item">
              ...
              <p class="upcoming-list__date">${w(e)}</p>
            </li>
            <li class="upcoming-list__item">
  ...
  </div>`}function k(t){b.innerHTML=t}const c=document.querySelector("#upcoming-btn");L().then(async t=>{const s=new Date,o=s.getMonth()+1,e=t.filter(a=>{const i=new Date(a.release_date),r=i.getMonth()+1;return i>s&&r===o});if(e.length===0){console.log("No upcoming films this month.");return}const n=Math.floor(Math.random()*e.length),p=e[n].genre_ids,g=await S(p),u=D(e[n],g);k(u),c.style.display="block";function l(){const a="upcomingFilms";(JSON.parse(localStorage.getItem(a))||[]).findIndex(d=>d.id===e[n].id)===-1?(c.classList.remove("remove-btn"),c.textContent="Add to my library"):(c.classList.add("remove-btn"),c.textContent="Remove from my library")}const m="upcomingFilms";c.addEventListener("click",a=>{a.preventDefault();let i=JSON.parse(localStorage.getItem(m))||[];const r=i.findIndex(d=>d.id===e[n].id);r===-1?i.push(e[n]):i.splice(r,1),localStorage.setItem(m,JSON.stringify(i)),l()}),l()});const _=document.querySelector(".weelky-trends-list");async function F(){try{const t=await y();_.innerHTML=await v(t,1),M(_),x(_)}catch(t){console.error(t)}}F();
