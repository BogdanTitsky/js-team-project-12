import{a as _,f}from"./footer-94a0366f.js";import{g as h,a as y,m as v}from"./card-markup-5dcd54b4.js";const x=document.querySelector(".js-upcoming"),a=document.querySelector("#upcoming-btn");async function L(){const e="https://api.themoviedb.org/3/movie/upcoming?api_key=183c3cacc9c38c09c14d38798ccfe9d7";try{const{data:n}=await _.get(e);return n}catch(n){console.error(n)}}async function k(e){try{return(await h()).filter(t=>e.includes(t.id)).map((t,i)=>i===0?t.name:t.name.toLowerCase()).join(", ")}catch(n){console.log(n)}}L().then(async({results:e})=>{const n=new Date,s=e.filter(o=>n<Date.parse(o.release_date));if(s.length===0){console.log("No");return}const t=Math.floor(Math.random()*s.length),i=s[t].genre_ids,l=await k(i),m=S(s[t],l);b(m);function r(){const o="upcomingFilms";(JSON.parse(localStorage.getItem(o))||[]).findIndex(u=>u.id===s[t].id)===-1?(a.classList.remove("remove-btn"),a.textContent="Add to my library"):(a.classList.add("remove-btn"),a.textContent="Remove from my library")}const g="upcomingFilms";a.addEventListener("click",o=>{o.preventDefault();let c=JSON.parse(localStorage.getItem(g))||[];const p=c.findIndex(u=>u.id===s[t].id);p===-1?c.push(s[t]):c.splice(p,1),localStorage.setItem(g,JSON.stringify(c)),r()}),r()});function w(e){return new Date(e).toLocaleString("uk",{day:"numeric",month:"numeric",year:"numeric"})}function S({backdrop_path:e,overview:n,popularity:s,release_date:t,title:i,vote_average:l,vote_count:m},r){return`
  ...
  </div>`}function b(e){x.innerHTML=e}const d=document.querySelector(".weelky-trends-list");async function $(){try{const e=await y();d.innerHTML=await v(e),f(d)}catch(e){console.error(e)}}$();
